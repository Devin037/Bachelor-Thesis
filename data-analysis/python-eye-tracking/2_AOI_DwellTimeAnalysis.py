...
def run_analysis_for_aoi(df, target_aoi_col, target_aoi_name, sd_thresh):
    """
    Calculates Proportional Dwell Time for a given AOI, removes outliers,
    ...
    # --- 1. Calculate Proportional Dwell Time ---
    print("\n[Step 1] Calculating Proportional Dwell Time...")
    # Isolate only fixation events
    ...
    # Group by trial and SUM the DURATION of fixations for the AOI
    dwell_times = aoi_fixations.groupby(['ParticipantID', 'classification_timeframe_number'])['Eye movement event duration'].sum().to_frame(name='Dwell_Time_ms').reset_index()

    # Calculate Total Trial Duration from the main df
    total_trial_durations = df.groupby(['ParticipantID', 'classification_timeframe_number'])['Eye movement event duration'].sum().to_frame(name='Total_Trial_Duration_ms').reset_index()
    ...
    # Merge AOI dwell times and total trial durations
    analysis_df = pd.merge(all_trials, dwell_times, on=['ParticipantID', 'classification_timeframe_number'], how='left')
    analysis_df = pd.merge(analysis_df, total_trial_durations, on=['ParticipantID', 'classification_timeframe_number'], how='left')

    # Fill NaNs and calculate the proportion
    analysis_df['Dwell_Time_ms'].fillna(0, inplace=True)
    analysis_df['Proportional_Dwell_Time'] = np.where(analysis_df['Total_Trial_Duration_ms'] > 0,
                                                      analysis_df['Dwell_Time_ms'] / analysis_df['Total_Trial_Duration_ms'],
                                                      0)
    print(f"Proportional Dwell Time calculated for {len(analysis_df)} trials.")

    # --- 2. Outlier Removal ---
    print(f"\n[Step 2] Checking for outliers in 'Proportional_Dwell_Time' for '{target_aoi_name}'...")
    ...
    analysis_df = remove_outliers_by_sd(analysis_df,
                                          ...
                                          value_col='Proportional_Dwell_Time',
                                          threshold=sd_thresh)
    ...
    output_filename = f"proportional_dwell_time_{target_aoi_name.replace(' ', '_').lower()}_results.csv"
    ...
    analysis_df['Proportional_Dwell_Time_Percent'] = analysis_df['Proportional_Dwell_Time'] * 100.0
    ...
    sns.barplot(x='Robot', y='Proportional_Dwell_Time_Percent', hue='difficulty', data=analysis_df, palette="viridis", capsize=.05, errorbar="se")
    plt.title(f"Mean Proportional Dwell Time on {target_aoi_name}\nby Condition and Difficulty")
    plt.ylabel('Mean Proportional Dwell Time (%)')
    plt.xlabel('Robotic Condition')
    plt.legend(title='Difficulty')
    # Save the plot to a file
    plot_filename = f"plot_{target_aoi_name.replace(' ', '_').lower()}.png"
    ...
    aov = pg.rm_anova(data=analysis_df,
                      dv='Proportional_Dwell_Time', 
                      within=['Robot', 'difficulty'],
                      subject='ParticipantID',
                      detailed=True)
    print(f"\n--- ANOVA Results for Proportional Dwell Time on '{target_aoi_name}' ---")
    pg.print_table(aov)
    ...
    if is_robot_sig or is_interaction_sig:
        print(f"\n--- Post-Hoc Tests for Proportional Dwell Time on '{target_aoi_name}' ---")
        posthocs = pg.pairwise_tests(data=analysis_df, dv='Proportional_Dwell_Time', within=['Robot', 'difficulty'], subject='ParticipantID', padjust='bonf')
        print(posthocs)
...
if __name__ == "__main__":
    ...
    main_df['Eye movement event duration'] = pd.to_numeric(main_df['Eye movement event duration'], errors='coerce')
    main_df.dropna(subset=['classification_timeframe_number', 'Eye movement event duration'], inplace=True)
    ...
    # The | operator works as a boolean OR for pandas columns.
    ...
    for aoi in aois_to_analyze:
        run_analysis_for_aoi(df=main_df.copy(), # Pass a copy to ensure original df is unchanged
